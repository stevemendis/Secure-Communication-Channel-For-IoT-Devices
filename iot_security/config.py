import os
from datetime import timedelta


class Config:
    ENV = 'production'
    SECRET_KEY = 'e0ad86e4a68d6aeef26cb571ea7a6524c3aebc825a452b375007dff823645e4f'
    SQLALCHEMY_DATABASE_URI = "postgres://postgres:password@localhost:5432/be_project"
    MAIL_SERVER = 'smtp.gmail.com'
    MAIL_PORT = 587
    MAIL_USERNAME = os.environ.get('MAIL_USERNAME' , 'authelectric@gmail.com')
    MAIL_PASSWORD = os.environ.get('MAIL_PASSWORD' , 'Moderncoe@123')
    MAIL_DEFAULT_SENDER = os.environ.get('MAIL_DEFAULT_SENDER', 'authelectric@gmail.com')
    MAIL_USE_TLS = os.environ.get('MAIL_USE_TLS', True)
    MAIL_USE_SSL = os.environ.get('MAIL_USE_SSL', False)
    REMEMBER_COOKIE_DURATION = timedelta(hours=24)
    ENCRYPT_KEY = 'ddc450603b1a0b804eb6bc3ec5a95f0e'
    ENCRYPT_IV = 'u\xa4\x94@\x9b\x08\xa6I\xe1w!E\xcb\xed\x934'
    # REMEMBER_COOKIE_PATH = 
    # REMEMBER_COOKIE_SECURE =
    # REMEMBER_COOKIE_HTTPONLY =
    RATELIMIT_DEFAULT = '50/hour;100/day;2000/year'
    RATELIMIT_STORAGE_URL = 'redis://localhost:6379'


'''
class ProductionConfig(Config):
    SQLALCHEMY_DATABASE_URI = os.environ.get('SQLALCHEMY_DATABASE_URI')
'''

class DevelopmentConfig(Config):
    ENV = 'development'
    DEBUG = False
    SECRET_KEY = 'e0ad86e4a68d6aeef26cb571ea7a6524c3aebc825a452b375007dff823645e4f'
    SQLALCHEMY_DATABASE_URI = os.environ.get('SQLALCHEMY_DATABASE_URI', Config.SQLALCHEMY_DATABASE_URI)
    SQLALCHEMY_TRACK_MODIFICATIONS = False
    RATELIMIT_DEFAULT = '10/hour;100/day;2000 per year'

    

'''
class TestingConfig(Config):
    DEBUG = False
    ENV = 'testing'
    TESTING = True
    SQLALCHEMY_DATABASE_URI = 'sqlite:///:memory:'
    SECRET_KEY = 'e0ad86e4a68d6aeef26cb571ea7a6524c3aebc825a452b375007dff823645e4f'
    WTF_CSRF_ENABLED = False
    MAIL_SUPPRESS_SEND = True
'''